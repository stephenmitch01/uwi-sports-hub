(function () {
   ...
   */
  async function init() {
    const session = await APP.mountSignedInShell({ active: "competitions", contextLabel: "Basketball Results" });
    if (!session) return;
    try {
      ...
      state.scorecards = normalizeArray(statLines).map(normalizeLine).filter(isBasketballScorecard).filter((line) => !state.teamId || String(line.teamId || line.statData?.uwiTeamId || "") === String(state.teamId));
      renderFilters();
      bind();
      render();
    } catch (error) {
      show(error?.message || "Basketball results could not be loaded.");
    }
  }
  ...
  function renderFilters() {
    const basketballCompetitions = state.competitions.filter((competition) => scorecardsFor(competition.id).length);
    els.tabs.innerHTML = `<button class="result-tab ${state.activeCompetitionId ? "" : "active"}" type="button" data-competition="">Matches (${state.scorecards.length})</button>${basketballCompetitions.map((competition) => `<button class="result-tab ${String(competition.id) === String(state.activeCompetitionId) ? "active" : ""}" type="button" data-competition="${escapeHtml(competition.id)}">${escapeHtml(competition.title || competition.name || "Competition")} (${scorecardsFor(competition.id).length})</button>`).join("")}`;
  ...
  function render() {
    if (!state.searchApplied) {
      ...
      els.grid.innerHTML = `<div class="empty-state">Click Search to view saved basketball score sheets.</div>`;
      return;
    }
    ...
    els.grid.innerHTML = rows.length ? rows.map(renderCard).join("") : `<div class="empty-state">No basketball results match those filters.</div>`;
  }
  ...
  function renderCard(line) {
    ...
    return `<article class="result-card">
      ...
      <div class="result-card-title">${escapeHtml(compactUwiTeamLabel(data.uwiTeamName))} ${escapeHtml(formatScore(data))} ${escapeHtml(opponentLabel(data))}</div>
      ...
      <div class="result-card-actions"><a href="basketball-scorecard-view.html?scorecardId=${encodeURIComponent(line.id)}">View Score Sheet</a><a href="basketball-scorecard.html?competitionId=${encodeURIComponent(line.competitionId)}&scorecardId=${encodeURIComponent(line.id)}">Edit Score Sheet</a><a href="competition-view.html?id=${encodeURIComponent(line.competitionId)}">Competition</a></div>
    </article>`;
  }
  ...
  function topPerformers(data) {
    ...
    const points = topBy(rows, "points");
    const rebounds = topBy(rows, "rebounds");
    const assists = topBy(rows, "assists");
    return [
      points ? `PTS: ${escapeHtml(points.name || "Player")} ${escapeHtml(points.points || 0)}` : "",
      rebounds ? `REB: ${escapeHtml(rebounds.name || "Player")} ${escapeHtml(rebounds.rebounds || 0)}` : "",
      assists ? `AST: ${escapeHtml(assists.name || "Player")} ${escapeHtml(assists.assists || 0)}` : ""
    ].filter(Boolean).join("<br>");
  }

  function topBy(rows, key) {
    const sorted = rows.filter((row) => Number(row[key] || 0) > 0).sort((a, b) => Number(b[key] || 0) - Number(a[key] || 0));
  ...
  function isBasketballScorecard(line) {
    return APP.normalizeSportSlug(line.sportSlug || line.sport) === "basketball" && (line.eventType === "scorecard" || line.statData?.eventType === "scorecard");
  }
  ...
  function classifyOutcome(data) {
    ...
    if (result.includes("tie")) return "tie";
  ...
  function mountRecentSearches() {
    ...
    document.querySelector("[data-recent-searches='basketball-results']")?.remove();
    els.searchButton.parentElement.insertAdjacentHTML("afterend", APP.renderRecentSearches("basketball-results"));
    document.querySelectorAll("[data-recent-searches='basketball-results'] [data-recent-search-index]").forEach((button) => button.addEventListener("click", function () {
      const item = APP.readRecentSearches("basketball-results")[Number(this.dataset.recentSearchIndex)];
      if (!item) return;
